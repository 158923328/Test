...
def main():
    ...
    def get_weather_data(): #获取网站数据
        ...
        if weather_dict.get('desc') == 'invilad-citykey':
            print(messagebox.askokcancel("提示","城市名有误，请重新输入"))
        else:
    ...
    def show_data(weather_dict, city_name): #显示数据
        ...
        Label(root1,text = "棋",fg = "green",bg = "yellow").place(x=10,y=255,width=125,height=20) #作者
        Button(root1, text='确认并退出', width=10, command=root1.quit).place(x=500, y=230, width=80, height=40)  # 退出按钮
        root1.mainloop() # 一旦检测到事件，就刷新绑定的按钮组件
